...
for f in ./*.bed; do  mv -- "$f" "${f%.*.1.gene.gff3.bed}.fa.sm.fna.bed"; done
...
bedtools getfasta -fi $i -bed $i.flank.bed -s -name -fo $i.pr.fa

    done

#in sankemake add dirctory for outputs
rm -r *.genome  *.bed  *.fai
