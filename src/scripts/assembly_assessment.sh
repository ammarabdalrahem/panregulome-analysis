...
assembly-stats -u out/uncom_data/Bdistachyon*.fa|cut -f 1-2,7,8> stats.csv

#convert output to CSV file
cat stats.csv |tr "\\t" "," > stats_table.csv

#loop for count and cut genes numbers  
for i in out/uncom_data/Bdistachyon*.gff3; do
...
#join two tables  
paste -d , stats_table.csv genes_table.csv > assessment_table.csv

#clear data
mv assessment_table.csv  tables/
