...
#calculate the ratio: (poor number /total nuber )*100
promoter_local_data$ratio_pr <- (promoter_local_data$poor_number_local_pr / promoter_local_data$total_number_pr)*100

#remove total and poor number keep only ratio col.
promoter_local_data[2:3] <- NULL   
...
#calculate the ratio: (poor number /total nuber )*100
cds_local_data$ratio_cds <- (cds_local_data$poor_number_local_cds / cds_local_data$total_number_cds)*100

#remove total and poor number keep only ratio col.
cds_local_data[2:3] <- NULL   



#merge all data for local alighment promoter and CDS
poor_seq_ratio_local <- merge(promoter_local_data,cds_local_data, by ="cluster.name",all=TRUE)

#merge all data with occupancy number

poor_seq_ratio_local <- merge(poor_seq_ratio_local,occupancy_number, by ="cluster.name",all=TRUE)
#remove any space
poor_seq_ratio_local <- apply(poor_seq_ratio_local, 2, str_remove_all, " ") 
poor_seq_ratio_local <-  as.data.frame(poor_seq_ratio_local)

#melt all
poor_seq_ratio_local <- melt(poor_seq_ratio_local, measure.vars = c("ratio_pr","ratio_cds"))

# define coloums headers
colnames(poor_seq_ratio_local)[4] <- "ratio"

#convert ratio value to numeric
poor_seq_ratio_local$ratio <- as.numeric(as.character(poor_seq_ratio_local$ratio))

#make order to x axis depend on levels orders
poor_seq_ratio_local$occupancy<- factor(poor_seq_ratio_local$occupancy, levels=(data_levels)) 


#plotting data
ggplot(poor_seq_ratio_local)+(aes(x=occupancy, y=ratio,color=variable,fill=variable)) + 
  geom_boxplot() +
  theme(strip.text.x = element_text(size =15))+
  xlab("Occupancy")+ ylab("Percentage of poor sequance")+
  theme(strip.text.x = element_text(size =15))+
